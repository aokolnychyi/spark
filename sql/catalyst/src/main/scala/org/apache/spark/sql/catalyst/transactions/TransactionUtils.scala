...
object TransactionUtils {
  ...
  def executeAndAbortOnFailure[R](txn: Transaction, func: => R): R = {
    try ActiveTransaction.withActiveTransaction(txn)(func)
    catch {
      case e: Throwable =>
        abort(txn)
        throw e
  ...
  def getOrBeginTransaction(catalog: TransactionalCatalogPlugin): Transaction = {
    ActiveTransaction.get match {
      case Some(txn) =>
        assert(txn.catalog.name == catalog.name)
        txn
      case None =>
        beginTransaction(catalog)
    }
  }

  private def beginTransaction(catalog: TransactionalCatalogPlugin): Transaction = {
    ...
    if (txn.catalog.name != catalog.name) {
      ...
      throw new IllegalStateException(
        s"Transaction catalog name (${txn.catalog.name}) must match " +
        s"original catalog name (${catalog.name}).")
    }
    txn
  }
}
