...
class UnresolveTransactionRelations(val catalogManager: CatalogManager)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan match {
    case TransactionalWrite(catalog) => unresolveRelations(plan, catalog)
    case _ => plan
  }

  private def unresolveRelations(
      plan: LogicalPlan,
      catalog: CatalogPlugin): LogicalPlan = allowInvokingTransformsInAnalyzer {
    plan transform {
      case r: DataSourceV2Relation if isLoadedFromCatalog(r, catalog) => TableReference.create(r)
    }
  }
