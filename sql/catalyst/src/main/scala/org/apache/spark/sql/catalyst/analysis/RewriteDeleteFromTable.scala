  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan resolveOperators {
    case d @ DeleteFromTable(aliasedTable, cond) if d.resolved =>
      EliminateSubqueryAliases(aliasedTable) match {
        case DataSourceV2RelationTable(_: TruncatableTable) if cond == TrueLiteral =>
          // don't rewrite as the table supports truncation
          d

        case r @ DataSourceV2RelationTable(t: SupportsRowLevelOperations) =>
        ...
        case DataSourceV2RelationTable(_: SupportsDeleteV2) =>
